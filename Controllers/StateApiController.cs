...
public sealed class StateApiController : ControllerBase
{
    ...
    {
        var states = _stateService.GetStateList();
        return Ok(states);
    }
    ...
    {
        if (!ModelState.IsValid) return ValidationProblem(ModelState);
        var state = _stateService.UpdateState(StateID, request);
        return state == null ? NotFound("State not found.") : Ok(state);
    }
    ...
    {
        if (!ModelState.IsValid) return ValidationProblem(ModelState);
        var state = _stateService.DeleteState(StateID);
        return state == null ? NotFound("State not found.") : Ok(state);
    }

    [HttpPatch]
    [Route("{StateID:int}")]
    public IActionResult PatchState([FromBody] PatchStateRequest request, int StateID )
    {
        if (!ModelState.IsValid) return ValidationProblem(ModelState);
        var state = _stateService.PatchState(StateID, request);
        return state == null ? NotFound("State not found.") : Ok(state);
    }
}